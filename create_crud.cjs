...
const template = (entity, routeName, interfaceName) => `import { Head, useForm, router } from '@inertiajs/react';
...
interface Props {
    ${entity}s: ${interfaceName}[];
}

export default function ${interfaceName}Index({ ${entity}s }: Props) {
    const [dialogOpen, setDialogOpen] = useState(false);
    const [editing, setEditing] = useState<${interfaceName} | null>(null);
    ...
    const openEdit = (item: ${interfaceName}) => {
        setEditing(item);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (editing) {
            form.put(\`/admin/${routeName}/\${editing.id}\`, {
    ...
    const handleDelete = (item: ${interfaceName}) => {
        confirm({
            title: 'Delete ${interfaceName}?',
            description: \`Are you sure you want to delete "\${item.name_en}"?\`,
            confirmText: 'Delete',
            variant: 'danger',
            onConfirm: () => {
                router.delete(\`/admin/${routeName}/\${item.id}\`, {
    ...
    return (
        <AppLayout>
            <Head title="${interfaceName}s" />
            <ConfirmDialog />
            <div className="max-w-4xl mx-auto space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-bold">${interfaceName}s</h1>
                        <p className="text-muted-foreground">Manage ${interfaceName}s.</p>
                    </div>
                ...
                <Card>
                    <CardContent className="p-0">
                        <div className="divide-y">
                            {${entity}s && ${entity}s.length > 0 ? ${entity}s.map(item => (
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>{editing ? 'Edit' : 'Add'} ${interfaceName}</DialogTitle>
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between">
                                ...
                                <AutoTranslateButton 
                                    ...
                                    onTranslated={(text) => form.setData('name_en', text)} 
                                />
                            </div>
...
const targets = [
    { entity: 'type', routeName: 'credential-types', interfaceName: 'CredentialType', dir: 'resources/js/pages/admin/credential-types' },
    { entity: 'type', routeName: 'project-types', interfaceName: 'ProjectType', dir: 'resources/js/pages/admin/project-types' },
    { entity: 'category', routeName: 'project-categories', interfaceName: 'ProjectCategory', dir: 'resources/js/pages/admin/project-categories' },
    { entity: 'category', routeName: 'certificate-categories', interfaceName: 'CertificateCategory', dir: 'resources/js/pages/admin/certificate-categories' }
];

targets.forEach(t => {
    fs.mkdirSync(t.dir, { recursive: true });
    fs.writeFileSync(path.join(t.dir, 'index.tsx'), template(t.entity, t.routeName, t.interfaceName));
});
